...
const positiveNumber: number = countPositiveNumbers(numbers);
...
function findWordsStartingWithA(wordsList: string[]): string[] {
    let aWords = []; 
...
// Question 15
function countScoreExtremes(scores: number[]) {
  ...
  function removeFalseyValues(arr: any[]) {
    ...
    return arr.filter((element: any) => !!element);
  }
...
//  Quetion 18
function sumOfElements(arr: any[], even_or_odd: string) {
   
    if (even_or_odd !== "even" && even_or_odd !== "odd") {
      throw new Error("even_or_odd must be either 'even' or 'odd'");
    }
  
    const sum = arr.reduce((acc: any, curr: number) => {
  ...
  // Question 19
  function findElementIndex(arr: string | any[], element: number) {
...
// Question 20
  ...
  console.log("Product:", product); // Output: Product: 40
   ...
   function filterByLength(strings: any[], n: number) {
    ...
    return strings.filter((string: string | any[]) => string.length > n);
  }
...
function fahrenheitToCelsius(fahrenheitTemps: number[]) {
...
const todoList = [
  ...
  function logIncompleteTasks(list: { task: string; completed: boolean; }[]) {
...
function sortIntegers(arr: any[]) {
  ...
  const strings2: string[] = ["apple", "banana", "cherry"];
